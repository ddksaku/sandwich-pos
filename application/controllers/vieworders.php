...
class ViewOrders extends Person_controller
{
	...
	function index()
	{
		...
		$data['manage_table']=get_order_manage_table($config['per_page'], $this->uri->segment($config['uri_segment']) , $this , 1);
		$this->load->view('vieworders/manage',$data);
	}
	
    function change_mode()
    {
        $mode = $this->input->post('mode');
        echo get_order_manage_table($config['per_page'], $this->uri->segment($config['uri_segment']) , $this , $mode);
    }
	/*
		...
		else
		{
			if (($handle = fopen($_FILES['file_path']['tmp_name'], "r")) !== FALSE)
			{
				...
				while (($data = fgetcsv($handle)) !== FALSE) 
				{
					...
					$customer_data=array(
                    'deposit'=>$data[11],
                    'account_number'=>$data[12]=='' ? null:$data[11],
					'taxable'=>$data[13]=='' ? 0:1,
					);
